...
public class DeviceControlActivity extends Activity {
    ...
    private Button drawecg;
    View layout1, layout2;
    //第一次畫layout1
    boolean viewload2 = false;        //第一次畫layout2
    int layouttype=0;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        layout1 = inflater.inflate(R.layout.gatt_services_characteristics, null);
    ...
    /*
    private void displayData(String data) {        //接收資料
        if (data != null) {
            if(dataforNotify) {
                if(data.length() > 20) {
                    Log.d(TAG, "data.length :" + data.length());
                    try {
                        if (meanvaluestart) {       //初始化
                            String[] datas = data.split(" ");
                            for(int i=0;i < datas.length;i+=2)
                                meanvalue = meanvalue+Integer.parseInt(datas[i],16)*256 + Integer.parseInt(datas[i+1],16);

                            meanvaluecount++;
                            if (meanvaluecount >= 100) {
                                meanvaluestart = false;
                                meanvalue /= 500;
                                gotodraw();
                            }
                        } else {
                            String[] datas = data.split(" ");
                            for(int i=0;i < datas.length;i+=2)
                                bufferyv[datacount] =Integer.parseInt(datas[i],16)*256 + Integer.parseInt(datas[i+1],16);

                            if (datacount < axis_X_length)
                                datacount++;
                            else {
                                datacount = 0;
                                bufferyv = new int[bufferSize];
                            }
                        }
